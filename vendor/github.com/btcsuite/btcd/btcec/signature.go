...
func signEOSRFC6979(privateKey *PrivateKey, hash []byte, nonce int) (*Signature, error) {

	privkey := privateKey.ToECDSA()
	N := S256().N
	halfOrder := S256().halfOrder
	k := nonceEOSRFC6979(privkey.D, hash, nonce)
...
func nonceEOSRFC6979(privkey *big.Int, hash []byte, nonce int) *big.Int {
	if nonce > 0 {
		moreHash := sha256.New()
		moreHash.Write(hash)
...
// signRFC6979 generates a deterministic ECDSA signature according to RFC 6979 and BIP 62.
func SignRFC6979DSA(privateKey *PrivateKey, hash []byte) (*big.Int, *big.Int, error) {
...
func (sig *Signature) BCHSerialize() []byte {
