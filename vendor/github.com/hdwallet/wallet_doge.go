...
func (c *doge) SignRawTransaction(signIn *SignInput) (*SignResult, error) {
	var vins []OutPutItem
	...
	///////////////
	dest_addr, err := btcutil.DecodeAddress(signIn.DestAddr, &DOGEParams)
	if err != nil {
		return nil, err
	}
	...
	addrSrc, err := btcutil.DecodeAddress(signIn.SrcAddr, &DOGEParams)
	if err != nil {
		return nil, err
	}
...
func (c *doge) GetWalletAccountFromWif() (*WalletAccount, error) {
	wif := c.GetKey().Wif
	if len(wif) > 0 {
...
func (c *doge) GetWalletAccount() *WalletAccount {
