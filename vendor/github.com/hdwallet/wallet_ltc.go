...
func (c *ltc) SignRawTransaction(signIn *SignInput) (*SignResult, error) {
	var vins []OutPutItem
	...
	///////////////
	dest_addr, err := btcutil.DecodeAddress(signIn.DestAddr, &LTCParams)
	if err != nil {
		return nil, err
	}
	...
	addrSrc, err := btcutil.DecodeAddress(signIn.SrcAddr, &LTCParams)
	if err != nil {
		return nil, err
	}
...
func (c *ltc) GetWalletAccountFromWif() (*WalletAccount, error) {
	wif := c.GetKey().Wif
	if len(wif) > 0 {
		...
		isLtc := btcWif.IsForNet(&LTCParams)
		if isLtc == false {
			return nil, errors.New("key type error")
		}
		pk := btcWif.SerializePubKey()
		fmt.Println("pk : ", hex.EncodeToString(pk))
		address, err := btcutil.NewAddressPubKeyHash(btcutil.Hash160(pk), &LTCParams)
		if err != nil {
			fmt.Println("Wif err : ", err.Error())
			return nil, err
...
// func (c *ltc) GetWallet() *hpywallet.HPYWallet {
// 	return &hpywallet.HPYWallet{}
// }
